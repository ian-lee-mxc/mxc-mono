<script lang="ts">
    ...
    let statusIndicators: StatusIndicatorProp[] = [
        ...
        // },
        {
            ...
            onClick: (value: Status) => {
                window.open(
                    `${l2ExplorerUrl}/block/${value.toString()}`,
                    "_blank"
                );
            },
            tooltip:
        ...
        },
        {
            ...
            onClick: (value: Status) => {
                window.open(
                    `${l1ExplorerUrl}/block/${value.toString()}`,
                    "_blank"
                );
            },
            tooltip:
        ...
        try {
            statusIndicators.push({
                ...
                statusFunc: async (
                    ...
                    const stateVars = await getStateVariables(
                        provider,
                        address
                    );
                    return new Date(
            ...
            statusIndicators.push({
                ...
                watchStatusFunc: async (
                    ...
                    contract.on(
                        ...
                        (
                            ...
                            if (
                                prover.toLowerCase() !==
                                oracleProverAddress.toLowerCase()
                            ) {
                                if (!provenAt.eq(0)) {
                                    onEvent(
                                        new Date(
                                            provenAt.toNumber() * 1000
                                        ).toString()
                                    );
                                }
                            }
                ...
                tooltip:
                    "The most recent block proof submitted on TaikoL1 contract.",
            });
...
<div class="text-center">
    <h1 class="text-2xl">Wannsee Protocol Status</h1>
</div>
<div
