...
contract TestERC20VaultMoonchain is TaikoTest {
    TaikoToken tko;
    ...
    function setUp() public {
        destChainId = CHAIN_ID_GENEVA;
        srcChainId = CHAIN_ID_ARBITRUM;
        ...
        tko = TaikoToken(
            deployProxy({
                ...
                data: abi.encodeCall(TaikoToken.init, (address(0), address(Carol)))
            })
        );
        tko.transfer(Alice, 100 ether);

        addressManager.setAddress(uint64(block.chainid), "taiko_token", address(tko));
        addressManager.setAddress(
            uint64(destChainId), "taiko_token", HARDCODE_MXC_NATIVE_TOKEN_ADDR
        );
        ...
        vm.deal(address(destChainIdBridge), 100_000 ether);
        ...
        addressManager.setAddress(srcChainId, "erc20_vault", address(erc20Vault));
        ...
        addressManager.setAddress(uint64(srcChainId), "bridged_erc20", bridgedERC20);
    ...
    function test_20Vault_send_erc20_no_processing_fee() public {
        ...
        erc20Vault.sendToken(
            ERC20Vault.BridgeTransferOp(
                destChainId, address(0), Bob, 0, address(erc20), 1_000_000 + 5_000_000, amount
            )
        );
    ...
    function test_20Vault_send_erc20_processing_fee() public {
        ...
        // CHANGE(MOONCHAIN): now allow send message value expect on moonchain network
        erc20Vault.sendToken{ value: fee }(
            ERC20Vault.BridgeTransferOp(
                destChainId,
                address(0),
                Bob,
    ...
    function test_20Vault_to_moonchain_with_mxc_native_token_src() public {
        vm.startPrank(Alice);

        uint64 amount = 1;

        uint256 AliceMxcBefore = tko.balanceOf(Alice);
        tko.approve(address(erc20Vault), amount);
        // Token with empty name succeeds
        erc20Vault.sendToken(
            ERC20Vault.BridgeTransferOp(
                destChainId, address(0), Bob, 0, address(tko), 1_000_000 + 5_000_000, amount
            )
        );

        assertEq(AliceMxcBefore - amount, tko.balanceOf(Alice));
        assertEq(amount, tko.balanceOf(address(erc20Vault)));
    }

    function test_20Vault_to_moonchain_with_mxc_native_token_dest() public {
        vm.startPrank(Alice);
        ...
        ERC20Vault.CanonicalERC20 memory mxcOriginalToken = ERC20Vault.CanonicalERC20({
            chainId: srcChainId,
            addr: address(tko),
            decimals: 18,
            symbol: "MXC",
        ...
        // Token with empty name succeeds
        destChainIdBridge.sendReceiveERC20ToERC20Vault(
            mxcOriginalToken, Alice, Bob, amount, bytes32(0), address(erc20Vault), srcChainId, 0
        );
        assertEq(BobEthBefore + amount, Bob.balance);
    }

    function test_20Vault_from_moonchain_with_mxc_native_token_src() public {
        //reverse chain id
        vm.chainId(destChainId);
        vm.startPrank(Carol);
        register(address(addressManager), "bridge", address(bridge), destChainId);
        register(address(addressManager), "bridge", address(destChainIdBridge), srcChainId);
        vm.startPrank(Alice);
        ...
        IBridge.Message memory _msg = destChainIdERC20Vault.sendToken{ value: fee + amount }(
            ERC20Vault.BridgeTransferOp(
                srcChainId, address(0), Bob, fee, HARDCODE_MXC_NATIVE_TOKEN_ADDR, 1_000_000, amount
            )
        );
        console2.log("BalanceBefore", AliceBalanceBefore, Alice.balance);
        assertEq(AliceBalanceBefore - amount - fee, Alice.balance);
        assertEq(bridgeBalanceBefore + amount + fee, address(bridge).balance);
        assertEq(_msg.fee, fee);
        assertEq(_msg.value, 0);
        bytes memory targetData = abi.encodeCall(
            ERC20Vault.onMessageInvocation,
            abi.encode(
                ERC20Vault.CanonicalERC20({
                    chainId: uint64(block.chainid),
                    addr: HARDCODE_MXC_NATIVE_TOKEN_ADDR,
                    decimals: 18,
                    symbol: "MXC",
                    name: "MXC Token"
                }),
        ...
        assertEq(_msg.data, targetData);
    }

    function test_20Vault_from_moonchain_with_mxc_native_token_dest() public {
        vm.startPrank(Carol);
        addressManager.setAddress(destChainId, "bridge", address(bridge));
        addressManager.setAddress(srcChainId, "bridge", address(destChainIdBridge));

        tko.transfer(address(destChainIdERC20Vault), 100 ether);

        vm.startPrank(Bob);
        uint256 amount = 1;
        uint256 bobMxcBalanceBefore = tko.balanceOf(Bob);
        uint256 valutBalanceBefore = tko.balanceOf(address(destChainIdERC20Vault));
        // reserve bridge
        destChainIdBridge.setERC20Vault(address(destChainIdERC20Vault));
        destChainIdBridge.sendReceiveERC20ToERC20Vault(
            ERC20Vault.CanonicalERC20({
            ...
            uint64(amount),
            bytes32(0),
            address(destChainIdERC20Vault),
            destChainId,
            0
        );

        assertEq(bobMxcBalanceBefore + amount, tko.balanceOf(Bob));
        assertEq(valutBalanceBefore - amount, tko.balanceOf(address(destChainIdERC20Vault)));
    }
}
